...
export async function getVarieties(species_id){
    try {
        const response = await axios.get(`${config.apiUrl}/varieties/${species_id}`);
...
export async function getCategories(selection){
    ...
    let params = {}
    console.log("SPECEIS",selection)
    if(selection.species)
        params['species_id'] = selection.species.value

    if(selection.variety)
        params['variety_id'] = selection.variety.value

    try {
        const response = await axios.get(`${config.apiUrl}/categories`,{params : params});
...
export async function getRegions(selection){
    ...
    if(selection.category)
        params['category_id'] = selection.category.value
 
    try {
        const response = await axios.get(`${config.apiUrl}/regions`,{params :params});
...
export async function getMarkets(selection){
    ...
    if(selection.region)
        params['region_id'] = selection.region.value    

    try {
        const response = await axios.get(`${config.apiUrl}/markets`,{ params : params});
...
export async function getCotas(selection){
    ...
    try {
        const response = await axios.get(`${config.apiUrl}/products`,{ params : params});
