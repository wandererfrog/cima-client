...
const varietiesReply = { varieties: [{ id: 139, name: "Turina" }] };
...
beforeEach(() => {
  nock(`${config.apiUrl}`)
    ...
    .get("/varieties/26")
    .reply(200, varietiesReply);

  nock(`${config.apiUrl}`)
    .defaultReplyHeaders({ "access-control-allow-origin": "*" })
    .get("/categories?species_id=26")
    .reply(200, categoriesReply);

  nock(`${config.apiUrl}`)
    .defaultReplyHeaders({ "access-control-allow-origin": "*" })
    .get("/markets?species_id=26")
    .reply(200, marketsReply);

  nock(`${config.apiUrl}`)
    .defaultReplyHeaders({ "access-control-allow-origin": "*" })
    .get("/regions?species_id=26")
    .reply(200, regionsReply);
});

describe("StateFunction tests", () => {
  it("Should return correct state object for when selecting species", () => {
    ...
    return stateFunc.getSpeciesState(selection, data).then(data => {
